...
function applyLanguage(lang){
  ...
  if(typeof renderSportsManagementDashboard==='function') renderSportsManagementDashboard();
  if(typeof renderSportsDashboardResults==='function') renderSportsDashboardResults();
  if(typeof renderGeneralIndicator==='function') renderGeneralIndicator();
  if(typeof renderGrantWorkflow==='function') renderGrantWorkflow();
  if(typeof renderCategory==='function' && window.currentActivityCategory){
    renderCategory(window.currentActivityCategory);
  }
...
/* SAH V26.0 — light-only, expanded bilingual interface */
window.sahUiText=function(ar){
  return document.documentElement.lang==='en'
    ? translateLoose(String(ar||''),'en')
    : String(ar||'');
};

window.addEventListener('DOMContentLoaded',()=>{
  console.info('SAH build 26.0 loaded');
  document.documentElement.dataset.sahBuild='26.0';
  document.body.classList.remove('theme-dark');
